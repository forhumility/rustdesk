...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/humbledesk.exe')
    os.chdir('../..')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/humbledesk.exe')
        system2('mv target/release/humbledesk.exe target/release/RustDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\humbledesk.exe')
        else:
            print('Not signed')
        system2(
