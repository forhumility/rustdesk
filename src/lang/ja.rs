...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("install_tip", "UAC の影響により、RustDesk がリモートデスクトップ上で正常に動作しない場合があります。UAC を回避するには、下のボタンをクリックしてシステムに HumbleDesk をインストールしてください。"),
        ...
        ("Discovered", "発見済み"),
        ("install_daemon_tip", "起動時に HumbleDesk を開始するには、システムサービスをインストールする必要があります。"),
        ("Remote ID", "リモート ID"),
        ("Paste", "貼り付け"),
        ...
        ("Turned off", "オフになりました"),
        ("Language", "言語"),
        ("Keep HumbleDesk background service", "RustDesk バックグラウンドサービスを維持"),
        ...
        ("elevated_foreground_window_tip", "リモートデスクトップでフォーカスされているウィンドウの操作にはより高い権限が必要なため、マウスとキーボードが一時的に使用できなくなっています。リモートユーザーにウィンドウを最小化、または接続管理画面から権限を昇格するよう要求してください。この問題を回避するには、リモートコンピューターに HumbleDesk をインストールしてください。"),
        ("Disconnected", "切断しました"),
        ("Other", "その他"),
        ...
        ("still_click_uac_tip", "リモートデスクトップユーザーが HumbleDesk を実行する際に、UACを許可する必要があります。"),
        ...
        ("scam_text1", "もし、知らない相手から電話で HumbleDesk のインストールやサービスの開始を依頼された場合、作業を進めずに、すぐに電話を切ってください。"),
        ...
        ("preset_password_warning", "このエディションには、既定で固定パスワードが設定されています。このパスワードを知っているユーザーはあなたのデバイスを完全にコントロールできるため、そのような危険がある場合は直ちに HumbleDesk をアンインストールして下さい！"),
        ...
        ("upgrade_remote_rustdesk_client_to_{}_tip", "リモート側の HumbleDesk クライアントをバージョン {} 以上にアップグレードしてください！"),
